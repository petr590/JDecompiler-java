...
public class JavaMethod extends JavaClassElement {
	...
	private enum AutogeneratedState {
		/** Метод не считается сгенерированным компилятором, выводится всегда */
		NOT_AUTOGENERATED    (showingAllowed -> true),
		...
		/** Метод сгенерирован компилятором и при этом не должен выводиться даже с флагом {@link Config#canShowAutogenerated()} */
		AUTOGENERATED_HIDDEN (showingAllowed -> false);
		
		private final BooleanPredicate canShow;
		
		private AutogeneratedState(BooleanPredicate canShow) {
		...
		public boolean isAutogenerated() {
			return this != NOT_AUTOGENERATED;
		}
		
		public static AutogeneratedState valueOf(boolean value) {
			return value ? AUTOGENERATED : NOT_AUTOGENERATED;
		}
	}
	
	
	private final LazyLoadingValue<AutogeneratedState> isAutogenerated;
	
	private AutogeneratedState initIsAutogenerated(ClassInfo classinfo) {
		
		if(modifiers.isSyntheticOrBridge()) {
			return AutogeneratedState.NOT_AUTOGENERATED;
		}
		...
		if(descriptor.isStaticInitializer()) { // static {}
			return AutogeneratedState.valueOf(methodScope.isEmpty());
		}
		...
		if(descriptor.isConstructor()) {
			...
			if(thisClassType.isAnonymous()) {
				return AutogeneratedState.AUTOGENERATED;
			}
			
			if(classinfo.getModifiers().isEnum()) {
				// enum constructor by default
				return AutogeneratedState.valueOf(descriptor.argumentsEquals(ClassType.STRING, PrimitiveType.INT) &&
			...
			return AutogeneratedState.valueOf(descriptor.getArgumentsCount() - descriptor.getVisibleStartIndex(classinfo) == 0 &&
		...
		if( classinfo.isRecord() &&
			...
			if(operand instanceof RecordSyntheticOperation) {
				return AutogeneratedState.AUTOGENERATED_HIDDEN;
			}
